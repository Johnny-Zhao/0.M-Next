...
## v0.3 开局（2026-07-06，会话交接点）

- **v0.2 标尺**：业主已用产品写完 V3.3 一章并导出，口头确认"挺好"——tag v0.2 是否已打**待核对**（`git tag` 查看）。
- **在飞任务（新会话第一件事：逐一核对状态）**：
  1. 批次 3 打包卡（前端三小修：校验汇总全工作空间/冲突不弹红 toast/文案去内部码与 UUID）——已给业主提示词，是否已发 Codex/已交付待确认；
  2. T-V03-1 AI 抽取端点（后端）：POST /ai/extract 草稿→SubmitAIChangeSet 变更集；网关 https://api.openai.com/v1，模型默认 gpt-4o-mini，AI_GATEWAY_URL/KEY 走环境变量（.env，永不入 git/对话）——卡已给业主，进度待确认；
  3. 待发：v0.3 前端卡（AI 面板加"粘贴草稿"输入框接 extract 端点，复用既有逐条确认 UI）。
- v0.3 目标一句话：**文字进、模型出**——粘草稿→AI 抽成模块参数→人逐条确认入库，产品第四支柱成形。
